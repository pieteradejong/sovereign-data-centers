# Latvia - Sovereign Government Data Center Network
...
> `python3 model/capacity_model.py LV` to update the capacity numbers; edit
> `model/eu27_parameters.csv` or `model/scaling_rules.csv` and re-run the generator to update this file.

## 1. Working thesis

Latvia does not need to become technologically autarkic. It needs enough independently controlled compute,
...
adjusted below for what is structurally different about Latvia.

## 2. Starting point

| | |
|---|---|
| Population | 1.86 m (Eurostat, 1 Jan 2025) |
| GDP | EUR 43 bn (2025, current prices) |
| Public administration employment (NACE O) | 63 k (Eurostat LFS 2025) |
| Non-household electricity price | 136.1 EUR/MWh (Eurostat, band IC, 2025-S2) |
| Renewables in electricity | 55.5% (2024) |
| Land area | 63,290 km2 |
| Live hyperscaler regions in-country | 0 |
| Existing government / sovereign cloud | Latvian National Federated Cloud / LVDC - LVRTC + Interior Ministry IC + MoA DC + National Library; RRF-funded phase completed May 2026 (64 state systems hosted) |
| National digital identity (anchor workload) | eID card / eParaksts / Smart-ID |
| Internet exchange / cable landings | LIX Riga; LVRTC Latvia-Sweden Baltic cable |

Relative to the Dutch baseline: population x0.10, public administration x0.09,
GDP x0.04. Resulting design load: x0.11 the Dutch figure.
...
|---|---|---:|---:|---:|---:|
| Digital identity / eID card | Critical government | 1,900 | 0 | 0.5 | 1.5 |
| Core government applications | Government | 5,300 | 0 | 1.7 | 1.35 |
| Data platforms & analytics | Government data | 2,800 | 8 | 4.8 | 1.25 |
| AI / sovereign model serving | AI | 1,100 | 32 | 0.7 | 1.3 |
| Defense classified compute | Defense | 4,600 | 40 | 2.3 | 1.5 |
...
| Metric | Value |
|---|---:|
| Physical servers | 608 (CPU 417 / GPU 30 / storage 161) |
| Rack equivalents | ~19 |
| IT critical load | 1.0 MW |
| Facility load (PUE 1.25) | 1.3 MW |
| Facility design load (+20% headroom) | **1.5 MW** |
| Sites by capacity / recommended | 1 / **3** (minimum 3) |
| Average design MW per site | 0.5 MW |
| Total CAPEX | **EUR 36 m** (facility EUR 15 m, IT EUR 18 m, network EUR 3 m) |
| Annual energy | 11,055 MWh |
...
|---|---|---:|---:|---:|---:|---|
| Riga | Primary civil cloud | 40% | 0.6 | 8 | EUR 6 m | LVRTC LVDC estate, LIX; Daugava flood zoning. |
| Ventspils / Kurzeme | Sovereign secondary | 27% | 0.4 | 5 | EUR 4 m | Latvia-Sweden cable landing; furthest from the eastern frontier. |
| Valmiera / Vidzeme | Government / continuity | 22% | 0.3 | 4 | EUR 3 m | Inland separation; Estonia transit. |
| Out-of-country reserve | Strategic reserve | 10% | 0.1 | 2 | EUR 2 m | Follow the Estonian Data Embassy pattern - national-only model cannot close this gap. |
...
Borders Russia and Belarus; NATO frontline; Baltic subsea sabotage and GNSS jamming; no seismic; some Daugava/coastal flooding; desynchronised from BRELL 2025; import-dependent in dry years

## 8. Recommendations specific to Latvia

1. **Anchor on eID card.** Digital identity is the workload that, if it fails, stops every other
   government service; it belongs in the sovereign core first, active-active across at least two regions.
2. **Build on the existing estate, do not replace it.** Latvian National Federated Cloud / LVDC - LVRTC + Interior Ministry IC + MoA DC + National Library is the
...
- Site-size assumption: is the 12 MW planning unit right for Latvia, or should sites be smaller?
...
| | |
|---|---|
| Governing instrument | State Information Systems Law; VRAA operates the state cloud platform |
| Cloud certification | No national scheme; ISO 27001 |
| Data classification | Law on Official Secrets: Restricted / Confidential / Secret / Top Secret |
| Procurement route | Electronic Procurement System (EIS) central catalogue |
...
| | |
|---|---|
| Government cloud | Latvian National Federated Cloud / LVDC - LVRTC + Interior Ministry IC + MoA DC + National Library; RRF-funded phase completed May 2026 (64 state systems hosted) |
| Maturity | operational |
| Digital identity | eID card / eParaksts / Smart-ID |
| In-country commercial regions | 0 |
| Interconnection | LIX Riga; LVRTC Latvia-Sweden Baltic cable |
...
Against that starting point, the modelled sovereign core is **1.5 MW of design load across
3 site(s)**, or roughly 608 servers. The gap between what runs today and that
...
|---|---|---:|---:|---:|---:|---|
| 1 | Sovereign core | 167 | 0.3 | EUR 6 m | 17% | no |
| 2 | Security and defense | 273 | 0.7 | EUR 17 m | 65% | no |
| 3 | State record | 98 | 0.2 | EUR 6 m | 81% | no |
